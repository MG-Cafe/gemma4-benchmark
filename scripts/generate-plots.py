...
# =============================================================================
# MEASURED DATA — GPU 4× RTX Pro 6000 Blackwell (TP=4, FP8, prefix caching ON)
# vLLM v0.19.0 on g4-standard-192 (4× GPUs, 384GB GDDR7 total)
# =============================================================================
...
gpu_p90_qps = [3.791, 4.112, 4.541, 4.668, 5.035, 5.451, 6.444, 7.669, 8.787]
gpu_p90_burst = [3.158, 3.480, 5.568, 5.684, 5.106, 16.647, 21.651, 31.277]
...
tpu_p90_burst = [0.0, 3.725, 4.182, 4.400, 4.820, 5.758, 7.033]
...
def plot_01():
    ...
    conc = [d[0] for d in gpu_burst]
    tpot = [d[4] for d in gpu_burst]
    ax.plot(conc, tpot, 's-', color=GPU_COLOR, linewidth=2.5, markersize=10,
...
def plot_02():
    fig, ax = plt.subplots(figsize=(12, 7))
    conc = [d[0] for d in gpu_burst]
    ttft = [d[3] for d in gpu_burst]
    ax.plot(conc, ttft, 's-', color=GPU_COLOR, linewidth=2.5, markersize=10,
            label=GPU_LABEL, zorder=3)
...
def plot_03():
    fig, ax = plt.subplots(figsize=(12, 7))
    conc = [d[0] for d in gpu_burst]
    e2e = [calc_e2e(d[3], d[4]) / 1000 for d in gpu_burst]
    ax.plot(conc, e2e, 's-', color=GPU_COLOR, linewidth=2.5, markersize=10,
            label=GPU_LABEL, zorder=3)
...
def plot_04():
    fig, ax = plt.subplots(figsize=(12, 7))
    conc = [d[0] for d in gpu_burst]
    mean_thr = [d[2] for d in gpu_burst]
    ax.plot(conc, mean_thr, 's-', color=GPU_COLOR, linewidth=2.5, markersize=10,
            label=GPU_LABEL, zorder=3)
    tpu_conc = [d[0] for d in tpu_burst]
...
# =============================================================================
# Plot 5: QPS Sweep (GPU + TPU)
# =============================================================================
def plot_05():
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7))
    qps = [d[0] for d in gpu_qps]
    ttft = [d[3] for d in gpu_qps]
    tpot = [d[4] for d in gpu_qps]
    e2e = [calc_e2e(d[3], d[4]) / 1000 for d in gpu_qps]
    ax1.plot(qps, ttft, 's-', color=GPU_COLOR, linewidth=2.5, markersize=9, label='GPU TTFT', zorder=3)
    ax1.plot(qps, tpot, 's--', color=GPU_COLOR, linewidth=2, markersize=8, label='GPU TPOT', alpha=0.7, zorder=3)
...
def plot_06():
    ...
    width = 0.35
    gpu_ttft = [d[3] for d in gpu_burst]
    gpu_tpot = [d[4] for d in gpu_burst]
    ax1.bar(x - width/2, gpu_ttft, width, label='GPU 4×RTX', color=GPU_COLOR, alpha=0.85, zorder=3)
...
def plot_08():
    ...
    x = np.arange(len(scenarios))
    width = 0.35
    gpu_ttft_s = [gpu_burst[i][3] / 1000 for i in burst_idx]
    gpu_decode_s = [gpu_burst[i][4] * (OUTPUT_TOKENS - 1) / 1000 for i in burst_idx]
    ax.bar(x - width/2, gpu_ttft_s, width, label='GPU TTFT (prefill)',
           color=GPU_COLOR, alpha=0.9, zorder=3)
    ax.bar(x - width/2, gpu_decode_s, width, bottom=gpu_ttft_s,
           label='GPU Decode', color=GPU_COLOR, alpha=0.4, zorder=3, hatch='//')
...
def plot_09():
    ...
    # Panel 1: Single TTFT
    ax = axes[0,0]
    vals = [gpu_baseline['ttft_ms'], tpu_baseline['ttft_ms'], maas_burst[0][1]]
    ...
    # Panel 2: Single E2E — P90
    ax = axes[0,1]
    vals = [gpu_baseline['e2e_p90_s'], tpu_baseline['e2e_p90_s'], maas_baseline['e2e_p90_s']]
    bars = ax.bar(labels, vals, color=colors, alpha=0.85, edgecolor='white', linewidth=0.5, width=0.6)
    for b, v in zip(bars, vals):
    ...
    # Panel 3: Burst 20 E2E — P90
    ax = axes[1,0]
    vals = [gpu_p90_burst[6], tpu_p90_burst[6], maas_burst[6][7]]
    bars = ax.bar(labels, vals, color=colors, alpha=0.85, edgecolor='white', linewidth=0.5, width=0.6)
    for b, v in zip(bars, vals):
    ...
    # Panel 4: Burst E2E sweep — P90
    ax = axes[1,1]
    gpu_burst_n = [d[0] for d in gpu_burst]
    ax.plot(gpu_burst_n, gpu_p90_burst, 's-', color='#00ff88', linewidth=2, markersize=8, label='GPU P90 E2E')
...
def plot_10():
    ...
    fig.text(0.5, 0.02, '** MaaS: $0.15/M in + $0.60/M out (incl. 20K input cost)',
